# src/models/train.py (without SMOTE, with scale_pos_weight and threshold analysis)
...
wandb.init(project=WANDB_PROJECT, name="xgboost_no_smote_threshold_sweep")
...
def evaluate(model, X_test, y_test, X_train=None):
    ...
    for t, p, r in zip(thresholds[::20], precisions[::20], recalls[::20]):
        wandb.log({"threshold": t, "precision_t": p, "recall_t": r})
